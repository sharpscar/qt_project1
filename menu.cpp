    ...
    , ui(new Ui::menu)
{
    ...
    /**
     * @brief connect
     * 컵 콘 화면에서 메뉴선택화면으로 이동
     */
    connect(ui->btn_goback_optiontype_concup, &QPushButton::clicked, this, &menu::go_to_select_menu);

    /**
     * @brief connect
     * 파인트 이상 아이스크림에서 맛선택으로 이동
     */
    connect(ui->btn_select_turnon_optiontype_concup_2, &QPushButton::clicked, this, &menu::go_to_select_flavor);

    connect(ui->btn_goback_optiontype_concup_2, &QPushButton::clicked, this, &menu::go_to_select_menu);
...
void menu::cup_plus_minus()
{
    cup_count=0;
    //컵추가
    connect(ui->btn_cup_plus_cup, &QPushButton::pressed, this, [this](){

        this->cup_count++;
        ui->lineEdit_cup_quentity_2->setText(QString::number(this->cup_count));
        ui->stackedWidget_con_btn->setCurrentWidget(ui->page_cupquantity_turnon_btn_2);

    });
    //컵빼기
    connect(ui->btn_cup_minus_cup, &QPushButton::pressed, this, [this](){
        if(cup_count>0)
        {
            this->cup_count--;
            ui->lineEdit_cup_quentity_2->setText(QString::number(this->cup_count));
            ui->stackedWidget_cup_btn->setCurrentWidget(ui->page_cupquantity_turnoff_btn_2);
        }
        // if(cup_count ==0){
        //     ui->stackedWidget_cup_btn->setCurrentWidget(ui->page_cupquantity_turnoff_btn_2);
        // }
...
void menu::clicked_setting(){ //메뉴 선택 버튼 커넥트
    ...
    for(int i=0; i<btns.size(); i++){ //맛 선택 버튼 커넥트
        connect(this->btns[i], &MenuBtn::clicked, this, [this, i](){
            flavor_name = this->btns2[i]->get_menu_name();
...
void menu::handle_clicked(int index) {
    ...
    //0522 write pkt
    if(this->icecream_flag==2){
        //컵 선택하는 위젯으로 넘겨라
        ui->stackedWidget_main->setCurrentWidget(ui->page_option);
        ui->stackedWidget_optiontype->setCurrentWidget(ui->page_optiontype_cup);
    }
...
void menu::size_of_flavor(){ //창 넘어가는 즉시 사이즈 만큼 위젯 추가하기 잘 안됨 ㅜㅜ
    flavor_name = "";
    select_flavor *flavor_cart = new select_flavor(this, flavor_name);
    for(int i=0; i <MENUSIZE; i++){ //이녀석을 모르는 듯
        ui->stackedWidget_select_btn_flavor->setCurrentIndex(1);
        this->btn_flavor_cart.append(flavor_cart);
        ui->gridLayout_flavor->addWidget(this->btn_flavor_cart[f],0,col_flavor);
        f++;col_flavor++;
...
void menu::load_image_flavor(const QString &flavor_name) {
    QSqlQuery query(db);
    QString sql_ = QString("select menu_file_path from menus where menu_name = \"%1\"").arg(flavor_name); //메뉴명을 기준으로 사진 경로 받아오기
    if (query.exec(sql_) && query.next()) {
        imgPath = query.value(0).toString(); //경로 파싱
    }
    // flavor_cart->setObjectName(QString::number(btn_flavor_cart.size())); //인덱스 기준으로 객체 이름 부여함, 삭제하기 위해서예용~
}
...
void menu::go_to_select_flavor(){
    ...
    //컵콘 개수확인 여기서 ..
    qDebug() << "컵 카운트 (이번엔 파인트)"<< cup_count<<"\n";
    qDebug() <<"select_flag" << select_flag<<"\n";;
    ...
    for (int i=0; i<con_count; i++){
        ...
        ui->gridLayout_menu->addWidget(concup, 0, column_concup);
        ui->gridLayout_menu->setAlignment(Qt::AlignLeft);
        column_concup++;
    ...
    for (int j=0; j<cup_count; j++){
        ...
        ui->gridLayout_menu->addWidget(concup1, 0, column_concup);
        ui->gridLayout_menu->setAlignment(Qt::AlignLeft);
        column_concup++;
    }
}
